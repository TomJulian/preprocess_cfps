...
```
git clone https://github.com/TomJulian/preprocess_cfps.git
```

If you don't have miniconda yet, run something like:
```
...
```
# a example model run
A run where you wanted to flip your RE images horizontally (so they look like left eyes) might look like: 
```
python /location/you/have/stored/the/preprocessing/script/preprocess_cfps.py \
  ...
  --image_resize 1024
```
A run where you dont want to change the orientation of images might be:
```
python /location/you/have/stored/the/preprocessing/script/preprocess_cfps.py \
  --source_dir /where/your/original/images/are/stored/ \
  --out_dir /where/you/want/your/processed/images/to/go/ \
